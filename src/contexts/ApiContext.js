...
export const ApiProvider = ({ children }) => {
  const [apiData, setApiData] = useState(null); // Adatok az API-ból
  const [kategoriaData, setKategoriaData] = useState([]); // Kategóriaadatok
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  // Adatok GET kéréssel történő lekérése
  const getData = async (vegpont, fv) => {
    setLoading(true);
    setError(null);
    try {
  ...
  // Adatok POST kéréssel történő küldése
  const postData = async (vegpont, data) => {
    setLoading(true);
    setError(null);
    try {
  ...
  // Szűrési funkció
  const szures = (szuroErtek) => {
    const vegpont = `/products/category/${szuroErtek}`;
    getData(vegpont, setApiData);
    console.log("Szűrés végpontja:", vegpont);
  };

  useEffect(() => {
    getData("/products", setApiData); // Termékek lekérése az API-ból
    getData("/products/categories", setKategoriaData); // Kategóriák lekérése
  }, []);

  return (
    <ApiContext.Provider
      value={{ apiData, getData, postData, kategoriaData, szures, loading, error }}
    >
      {children}
    </ApiContext.Provider>
  );
